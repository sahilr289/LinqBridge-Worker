// worker.cjs — LinqBridge Worker (FINAL: connect flow untouched, messaging robust, nav fixed)
//
...
// - Mobile-first profile hop (env flag, fixed URL builder) + stable SPA-safe nav
...
// ---------- Profile nav (SPA-safe) ----------
function toMobileProfileUrl(u) { try { const url = new URL(u); url.hostname = "m.linkedin.com"; return url.toString(); } catch { return u; } }
function ensureTrailingSlash(u) { try { const url = new URL(u); if (!url.pathname.endsWith("/")) url.pathname += "/"; return url.toString(); } catch { return u; } }
function isProfileUrl(u) { try { return /https?:\/\/([^.]+\.)?linkedin\.com\/(m\/)?in\//i.test(u); } catch { return false; } }
function isFeedUrl(u) { try { return /https?:\/\/([^.]+\.)?linkedin\.com\/(m\/)?feed\/?([?#]|$)/i.test(u); } catch { return false; } }

// Use commit to avoid SPA stalls, then settle
async function gotoStable(page, url, timeout = NAV_TIMEOUT_MS) {
  let resp = null;
  try {
    resp = await page.goto(url, { waitUntil: "commit", timeout });
  } catch (e) {
    console.log(`[nav] goto(commit) error for ${url}:`, e?.message || e);
  }
  await page.waitForLoadState("domcontentloaded", { timeout: Math.min(10000, timeout) }).catch(()=>{});
  await waitFullLoad(page, Math.min(timeout, 15000));
  return resp;
}

function profileVariants(u) {
  const base = ensureTrailingSlash(u);
  return [ base, base + "about/", base + "details/experience/" ];
}

async function navigateProfileClean(page, rawUrl) {
  const primary = ensureTrailingSlash(String(rawUrl).replace("linkedin.com//", "linkedin.com/"));
  const mobile = toMobileProfileUrl(primary);
  const hostOrder = USE_PROFILE_MOBILE_FIRST ? [mobile, primary] : [primary, mobile];

  for (let hostIdx = 0; hostIdx < hostOrder.length; hostIdx++) {
    const hostUrl = hostOrder[hostIdx];
    for (const cand of profileVariants(hostUrl)) {
      try {
        console.log(`[nav] → profile try: ${cand}`);
        const resp = await gotoStable(page, cand, NAV_TIMEOUT_MS);
        const status = resp ? resp.status() : null;
        const finalUrl = page.url();
        console.log(`[nav] final after goto: ${finalUrl}`);

        const hard = await detectHardScreen(page);
        if (status === 429 || hard === "429") return { authed: false, status: 429, usedUrl: cand, finalUrl, error: "rate_limited" };
        if (hard === "404") return { authed: false, status: status || 404, usedUrl: cand, finalUrl, error: "not_found" };

        // Authwall recovery once
        if (/\/authwall/i.test(finalUrl)) {
          console.log("[nav] authwall detected → re-auth then retry once");
          try {
            await gotoStable(page, "https://www.linkedin.com/login", Math.min(15000, NAV_TIMEOUT_MS));
            await sleep(1200);
            await gotoStable(page, cand, NAV_TIMEOUT_MS);
          } catch (e) {
        ...
        if (hard2 === "404") return { authed: false, status: 404, usedUrl: cand, finalUrl: final2, error: "not_found" };
        if (hard2 === "429") return { authed: false, status: 429, usedUrl: cand, finalUrl: final2, error: "rate_limited" };

        // If bounced to feed, treat as soft fail and continue trying variants
        if (isFeedUrl(final2)) {
          console.log("[nav] bounced to feed; will try another profile variant/host");
          continue;
        }

        const authed = !(await isAuthWalledOrGuest(page));
        if (authed && isProfileUrl(final2) && !hard2) {
          return { authed: true, status: status || 200, usedUrl: cand, finalUrl: final2 };
        }

        // Not profile? Try next candidate.
      } catch (e) {
        console.log("[nav] profile try error:", e?.message || e);
      }
    }
  }
  return { authed: false, status: null, usedUrl: null, finalUrl: page.url(), error: isFeedUrl(page.url()) ? "redirected_to_feed" : "authwall_or_unknown" };
}

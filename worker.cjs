...
async function getFetch() {
  ...
  try {
    fetchRef = (await import("node-fetch")).default;
    return fetchRef;
  } catch {}
  try {
    fetchRef = require("cross-fetch");
    return fetchRef;
  } catch {}
...
async function anyVisible(...locs) {
  const checks = await Promise.all(
    locs.map((l) => l.first().isVisible({ timeout: 800 }).catch(() => false))
  );
...
async function briefProfileScroll(page, totalMs = 2000) {
  ...
  while (Date.now() - start < totalMs) {
    try {
      await page.mouse.wheel(0, within(80, 140));
    } catch {}
...
async function isAuthWalledOrGuest(page) {
  try {
    ...
    if (title.includes("sign in") || title.includes("join linkedin") || title.includes("authwall"))
      return true;
...
async function createBrowserContext({ headless, userStatePath }) {
  ...
  const storageStateOpt =
    (!FORCE_RELOGIN && userStatePath && fs.existsSync(userStatePath))
      ? userStatePath
...
async function ensureAuthenticated(context, page, userStatePath) {
  try {
    const r1 = await page.goto("https://www.linkedin.com/feed/", {
      waitUntil: "domcontentloaded",
      timeout: NAV_TIMEOUT_MS,
    });
  ...
  try {
    const r2 = await page.goto("https://m.linkedin.com/feed/", {
      waitUntil: "domcontentloaded",
      timeout: NAV_TIMEOUT_MS,
    });
  ...
  try {
    ...
    const r = await page.goto("https://www.linkedin.com/login", {
      waitUntil: "domcontentloaded",
      timeout: NAV_TIMEOUT_MS,
    });
...
function isProfileUrl(u) {
  try {
    return /https?:\/\/([^.]+\.)?linkedin\.com\/(m\/)?in\//i.test(u);
  } catch {
    return false;
  }
}
    ...
    try {
      ...
      if (i === tries.length - 1) {
        return {
          ...
          usedUrl: u,
          finalUrl,
          error: isAuthwall ? "authwall" : isProfileUrl(finalUrl) ? "authwall_or_unknown" : "not_profile",
        };
      }
    } catch (e) {
      const finalUrl = page.url();
...
async function sendConnectionRequest(page, note) {
  ...
  if (rs1.status === "connected")
    return { actionTaken: "none", relationshipStatus: "connected", details: "Already 1st-degree" };
  if (rs1.status === "pending")
    return { actionTaken: "none", relationshipStatus: "pending", details: "Invitation already pending" };

  let opened = await openConnectDialog(page);
  if (!opened.opened) {
    try {
      await page.mouse.wheel(0, within(600, 1000));
      await sleep(within(600, 1000));
    } catch {}
  ...
  if (completed.sent)
    return { actionTaken: "sent_without_note", relationshipStatus: "pending", details: "Invitation sent" };

  const rs2 = await detectRelationshipStatus(page);
  if (rs2.status === "pending")
    return { actionTaken: "sent_maybe", relationshipStatus: "pending", details: "Pending after dialog" };
  if (rs2.status === "connected")
    return { actionTaken: "none", relationshipStatus: "connected", details: "Connected" };
...
async function openMessageDialog(page) {
  ...
  // *** UPDATED: broaden selectors to include aria-label="Message <Name>" and artdeco text ***
  const direct = [
    // Role-based with accessible name containing "Message" (covers "Message" and "Message Anshul")
    page.getByRole("button", { name: /(^|\s)Message(\s|$)/i }),
    page.getByRole("link",   { name: /(^|\s)Message(\s|$)/i }),

    // aria-label variants like your sample ("Message", "Message Anshul", etc.)
    page.locator('button[aria-label^="Message"]'),
    page.locator('a[aria-label^="Message"]'),

    // visible-label fallbacks via artdeco text node
...
async function handleAuthCheck(job) {
  ...
  try {
    ...
    if (auth.ok) {
      ...
      if (video) {
        try { console.log("[video] saved:", await video.path()); } catch {}
      }
...
async function handleSendConnection(job) {
  const p = job?.payload || {};
  const targetUrl =
    p.profileUrl || (p.publicIdentifier ? `https://www.linkedin.com/in/${encodeURIComponent(p.publicIdentifier)}/` : null);
  ...
  try {
    ...
    if (hard === "404" || hard === "429" || hard === "captcha") {
      const details =
        hard === "404" ? "Public profile URL returned 404."
...
async function processOne() {
  ...
  try {
    ...
    switch (job.type) {
      case "AUTH_CHECK":
        result = await handleAuthCheck(job);
        break;
